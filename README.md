...
### Into a called (reusable) workflow

The store carries values into a workflow the run *calls*, in that same run. A called
workflow is a separate job on a separate runner, so `$GITHUB_ENV` does not reach it and
`needs.<job>.outputs` would mean wiring every value through by hand. `load-shared: true`
reads them all back under their own names.

**The caller** computes the value, publishes it, then calls the shared workflow:
...
jobs:
  ...
  shared-vars:
    ...
    permissions:
      contents: read
      actions: read
    uses: owner/repo/.github/workflows/shared-workflow.yml@main
```

**The called workflow** reads them back — no inputs, no `secrets: inherit`, no checkout:
...
- **`needs: build`.** Ordering. Without it both jobs start together and the called
  workflow reads a store the build has not written yet.
- **`permissions:` on the *calling* job.** A called workflow's `GITHUB_TOKEN` gets what
  the calling job grants it, so `actions: read` belongs there; the called workflow's own
  jobs run with that grant and need no block of their own.
- **No `actions/checkout` in the called workflow.** `load-shared: true` reads the store
  over the API. Check out only when a committed `am-build-vars.yml` should apply too.

Both the repository the store lives in and its default scope (`github.ref_name`) come
from the **caller** — a called workflow runs in the caller's context — so the two sides
land on the same store even when the shared workflow lives in another repository, as
above.

> A reusable workflow is called at the **job** level, with `jobs.<id>.uses`. It cannot be
> a step: `uses:` inside `steps:` takes an *action*, and pointing it at a workflow file
> fails with `Can't find 'action.yml', 'action.yaml' or 'Dockerfile'`. To run shared steps
> inside the *same* job — where the variables are already in the environment and the store
> is not needed at all — write them as a
> [composite action](https://docs.github.com/actions/sharing-automations/creating-actions/creating-a-composite-action)
> instead.
